...
tileGen = np.random.poisson(water_prob, map_size[0]*map_size[1]).reshape(map_size[0], map_size[1])
        ...
        if gen[n, m] > 0:
            for x in range(-5,6):
                for y in range(-5,6):
                    try:
                        prob = random.random()
                        if prob < 0.02 and map[n+x, m+y, 2] == grassTile_id:
                            terrain[n + x, m + y] = woodTerrain_id
                        elif prob < 0.06 and map[n+x, m+y, 2] == grassTile_id:
